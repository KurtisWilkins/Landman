...
def test_defaults_return_reference_scenario(client: TestClient) -> None:
    ...
    body = r.json()
    assert abs(float(body["deal"]["irr"]) - 0.18639) < 0.001
    ...
    labels = {body["partner"]["label"], body["rjourney"]["label"], body["deal"]["label"]}
    assert labels == {"Partner Equity", "RJourney Equity", "Deal-Level"}
...
def test_override_stream_is_used(client: TestClient) -> None:
    ...
    assert r.status_code == 200
    body = r.json()
    assert [float(x) for x in body["deal_cashflows"]] == [-150000000, 0, 0, 0, 0, 230000000]
    assert 0.08 < float(body["deal"]["irr"]) < 0.15
