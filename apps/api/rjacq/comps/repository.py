...
async def list_comps(session: AsyncSession, deal_id: str) -> Sequence[Comp]:
    stmt = select(Comp).where(Comp.deal_id == deal_id).order_by(Comp.distance_mi)
    return (await session.execute(stmt)).scalars().all()


async def assign_amenity_ranks(session: AsyncSession, deal_id: str) -> None:
    """Rank comps 1..N by amenity_score (desc); comps without a score are left unranked."""
    comps = [c for c in await list_comps(session, deal_id) if c.amenity_score is not None]
