...
async def get_proforma(
    ...
    """Pro forma results."""
    not_implemented("GET /deals/{id}/proforma", phase="Phase 2 (underwriting)")


@router.patch("/deals/{deal_id}/assumptions", response_model=ProformaResults)
async def override_assumption(
    deal_id: str,
    _body: AssumptionOverride,
    _principal: Principal = Depends(require(Capability.ASSUMPTION_OVERRIDE)),
) -> ProformaResults:
    """Override an assumption (records author + note) and recalculate."""
    not_implemented("PATCH /deals/{id}/assumptions", phase="Phase 2 (underwriting)")
